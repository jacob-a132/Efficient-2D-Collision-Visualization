...
function setDots(numPoints = 100, minRadius = 3, maxRadius = 6, setNumPoints) {
...
function App() {
  ...
  const [minRadius, setMinRadius] = useState(3);
  ...
  useEffect(() => {
    ...
    let newIntervalId = setInterval(() => {
      ...
      for (let i = 0; i < numDots; i++){
        ...
        // const neighbourIds = tempqt.getNeighbourIds(entity);
        // neighbourIds.forEach(id => {
        //   if(areColliding({...entity, x: newX, y: newY}, tempEntities[id])){
        //     directions[i] = (directions[i] + 2) % 4;
        //     directions[id] = (directions[id] + 2) % 4;
        //     newX = [1,2].includes(directions[i]) ? entity.x+stepX : entity.x-stepX;
        //     newY = [2,3].includes(directions[i]) ? entity.y+stepY : entity.y-stepY;
        //   }
        // });
